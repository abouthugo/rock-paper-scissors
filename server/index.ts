...
io.on("connection", (player) => {
	...
	player.on("requestMatch", (id) => {
		...
		try {
			const sender = find(player.id, connectedUsers);
			const receiver = find(id, connectedUsers);
			logger.info(`${sender?.name} is trying to battle with ${receiver?.name}`);
	...
	player.on("heartbeat", () => {
		...
		try {
			const user = find(player.id, connectedUsers);
			if (!user) return;
